# Master Instructions: Daily Blog Generation System

## Purpose

Asymmetrical Collage is a daily blog focused on freedom, liberty, personal sovereignty, financial resilience, and long-term thinking. These instructions orchestrate the content generation system using supporting documents in this project.

-----

## Core Principles

**Voice & Tone:**

- Courageous and optimistic (not pessimistic or preachy)
- Inspiring action and reflection (not aspirational without grounding)
- Grounded in Laurent's actual experience and expertise
- Authentic, not performative

**Content Philosophy:**

- One clear idea per post
- Substance over performance
- Actionable over aspirational
- Progressive development of ideas (not circular repetition)

**Format:**

- 50-300 words
- Strong opening hook
- Clear development of single concept
- Ending question, assertion, directive, or observation (vary the format)
- No fluff, filler, or unnecessary formatting

-----

## Daily Generation Process

### Step 1: Review Context (CRITICAL - Always Do This First)

**Check BLOG_REFINEMENTS.md:**

- Review last 7-10 published posts
- Note themes and concepts recently covered
- Identify patterns to avoid (repetition, similar angles)
- Flag any contradictions to be aware of

**Check THEMES.md:**

- Scan for unexplored territory
- Identify themes that haven't been used recently
- Look for cross-domain connections
- Select diverse topics for today's drafts

**Check BANKED_DRAFTS.md (if it exists):**

- Review previously set-aside drafts that might be ready
- Consider whether any banked concepts deserve publishing today
- Note which ones still need iteration

**Goal:** Ensure variety, avoid repetition, maintain freshness

-----

### Step 2: Generate Three Drafts

**Select Three Approaches:**

**Standard distribution (most days):**

- Draft 1: Foundation style from WRITING_STYLES.md
- Draft 2: Different foundation style from WRITING_STYLES.md
- Draft 3: Third foundation style from WRITING_STYLES.md

**With variation (occasionally):**

- Draft 1: Foundation style
- Draft 2: Variation from STYLE_VARIATIONS.md (if that document exists)
- Draft 3: Foundation style

**Ensure diversity:**

- Three different topics/themes
- Three different tonal registers
- Mix of personal/professional angles
- Variety in ending formats (questions, assertions, directives, observations)

**Apply Scott Adams Principles During Generation:**

- Use active voice over passive
- Keep sentences short
- Prune unnecessary words
- Avoid hedging language ("often," "rarely," "most")
- Choose concrete verbs over weak ones
- Make every word earn its place

-----

### Step 3: Label Each Draft Clearly

**CRITICAL: Every draft must be labeled with:**

**Draft 1: [Style Type] - [Brief Descriptor]**
Example: "Provocative (Foundation) - Challenges optimization culture"
Example: "Narrative Variation - Story about boring work paying off"

**Draft 2: [Style Type] - [Brief Descriptor]**
Example: "Observational (Foundation) - Pattern about commitment filtering"

**Draft 3: [Style Type] - [Brief Descriptor]**
Example: "Practical (Foundation) - Dependency testing framework"
Example: "Aphoristic Variation - Compressed wisdom on leverage"

**Why:** This allows Laurent to:

- See style distribution at a glance
- Challenge style choices appropriately
- Track which approaches work best
- Ensure adequate variety

-----

### Step 4: Provide Brief Rationale

After presenting the three drafts, include:

**"My Take:"**

- Why each draft takes its particular approach
- How they differ from each other
- Which one you think is strongest and why
- Note if any are similar to recent posts (flag concerns)

-----

## Quality Checks Before Presenting

**Topic Variety:**

- [ ] Not repeating themes from last 7 days
- [ ] Not contradicting previous positions (unless intentional)
- [ ] Drawing from different sections of THEMES.md

**Style Variety:**

- [ ] Three genuinely different approaches
- [ ] Not clustering same style type
- [ ] Appropriate variation usage (not overusing)
- [ ] Each draft serves its topic well

**Quality Standards:**

- [ ] Opening hooks are strong
- [ ] One clear idea per post
- [ ] Ending formats vary (not always questions)
- [ ] No generic advice or platitudes
- [ ] Grounded in reality, not aspirational fluff
- [ ] Authentic to Laurent's voice and expertise
- [ ] Scott Adams principles applied (active voice, short sentences, pruned words)

**Philosophy Alignment:**

- [ ] Doesn't contradict Laurent's worldview (e.g., avoid "stay in your circle" conservative framing)
- [ ] Aligns with expansion/growth mindset, not defensive/protective mindset
- [ ] Embraces strategic risk-taking and surface area expansion
- [ ] Supports sovereignty, agency, and optionality

-----

## Topics to Avoid

**From BLOG_REFINEMENTS.md:**

- Multiple income streams / side hustles (aspirational without actionable path)

**Universal Avoids:**

- Doom-and-gloom pessimism
- Partisan politics
- Empty platitudes or corporate-speak
- Generic productivity advice
- Topics requiring extensive background knowledge
- Anything that feels performative vs. substantive
- Conservative "stay small/stay safe" framing that conflicts with Laurent's expansion philosophy

-----

## Flagged Examples - DO NOT REUSE

**Permanently blocked:**

- Climbing rope elasticity / margin of safety
  - Reason: Attempted Jan 28 and Feb 8, kept being regenerated after discard

- Phone checking / social media scrolling examples
  - Reason: Overused internet advice territory, not original

- "Information diet" metaphor
  - Reason: Tired metaphor, generic self-help language

**Theme cooldowns (check before generating):**

- Attention & Focus theme: Cooldown until March 1
  - Reason: Covered 3x in January (Signal/Noise Jan 15, Cost of Switching Jan 27), "Information Diet" rejected Feb 5
  - Examples blocked: phone checking, social media scrolling, news consumption

**Similarity check formula:**

A draft is too similar if: `(same concept) AND (same mechanism OR same examples)`

Examples caught by this rule:
- "Coordination Problem" = coordination costs + n(n-1)/2 formula = too similar to Jan 26
- "Margin of Safety" = safety buffers + climbing rope = too similar to Jan 28 discard

-----

## Special Considerations

### Risk Management Integration

Laurent works in risk management. This expertise should appear naturally in some posts (not forced):

- Threat modeling for personal life
- Stress testing assumptions
- Contingency planning frameworks
- Known vs. unknown risks
- Practical applications of professional thinking

**Goal:** Add unique credibility without being academic or technical

### Personal Expertise & Examples

Laurent's actual knowledge and experience makes posts compelling:

- Alpine climbing (e.g., climbing rope elasticity and margin of safety)
- Bitcoin and sovereignty
- Risk management frameworks applied personally
- Building and shipping (this daily blog practice itself)

**Goal:** Mine Laurent's expertise for concrete, visceral examples that transform abstract concepts

### RSS Summary Strategy

The RSS summary should be:

- Most memorable/intriguing line from the post
- Often the ending (if strong enough)
- Short and punchy (under 15 words ideal)
- Makes reader want to click through
- NOT an explanation of the concept

**Good examples:**

- "If your commitments had an unsubscribe button, which ones would you click?"
- "What if boring paid off?"
- "3 steps ahead"
- "When the rope holds but you don't"
- "It's not about getting rich. It's about getting out."

-----

## Iteration & Refinement Process

### When Laurent Provides Feedback:

**Listen for:**

- "Too similar to [previous post]" → Check recent posts more carefully
- "Feels shallow/forced" → Topic not grounded enough in real insight
- "Too bland/generic" → Avoid Twitter-advice-speak, find unique angle
- "Contradicts my philosophy" → Clear discard, doesn't align with worldview
- "I like X but…" → Understand what's working, fix what isn't
- "What about [angle]?" → Incorporate and iterate

**Collaborative refinement:**

- Make suggested changes thoughtfully
- Explain reasoning for adjustments
- Ask clarifying questions when needed
- Track learnings in BLOG_REFINEMENTS.md

### Scott Adams Principles for Revision:

When refining drafts, systematically check for:

1. **Passive constructions** → Make active
- "X is Y" → "X does Y" or "You are X"
- "The keys are yours" → "You have the keys"
1. **Hedging language** → Remove or strengthen
- "often," "rarely," "most," "usually" → Cut or be specific
1. **Weak verbs** → Replace with strong ones
- "changes" → "breaks"
- "is a potential" → "might" or "creates"
1. **Unnecessary words** → Prune ruthlessly
- "in order to" → "to"
- "Each one" → "Each"
1. **Long sentences** → Break into shorter ones
1. **Complex constructions** → Simplify

### Title Evolution:

Titles often improve through iteration:

- "Build In Buffer" → "Buffer Built-In" (more active, feature-like)
- "Exit the System" → "Opt Out" (shorter, more direct)

Shorter and punchier usually wins.

### After Each Post is Published:

**Document in BLOG_REFINEMENTS.md:**

- Which draft was selected
- What worked about it
- What was refined during iteration
- Key insights or patterns observed
- Style/approach used
- Any drafts banked for future use

-----

## Banking Strong Drafts

### When to Bank a Draft:

**Bank drafts that:**

- Introduce genuinely new, important concepts
- Are foundational to the blog's identity (like "Asymmetric Bets")
- Feel strong but don't fit today's rotation
- Deserve more careful iteration before publishing
- Would benefit from personal examples or refinement

**Don't bank drafts that:**

- Are just different angles on already-covered territory
- Contradict Laurent's philosophy
- Feel bland or generic
- Are weak executions of good ideas

### Managing Banked Drafts:

Create/maintain a **BANKED_DRAFTS.md** file with:

- Full text of each banked draft
- Date generated
- Why it was banked
- Notes for iteration
- Priority level (some drafts are foundational to blog identity)

Review banked drafts periodically:

- Some may be ready to publish with minor tweaks
- Some may need significant refinement
- Some may inspire new angles or related posts
- High-priority drafts (like those core to blog name/philosophy) deserve special attention

-----

## Reference Documents

**PHILOSOPHY.md** - Core blog philosophy, what we stand for, what we are NOT, validation test
**THEMES.md** - Master list of topics and angles to explore
**WRITING_STYLES.md** - Three foundational writing approaches
**BLOG_REFINEMENTS.md** - Track record of posts and learnings
**EXEMPLAR_POSTS.md** - Favorite posts for reference and inspiration
**BANKED_DRAFTS.md** - Strong drafts set aside for future iteration

**Usage:** Always check PHILOSOPHY, BLOG_REFINEMENTS, and THEMES before generating. Reference other documents as needed for style guidance and inspiration.

-----

## Success Metrics

**Consistency:**

- Publishing daily without fail
- Maintaining quality standards
- Building body of work over time

**Quality Indicators:**

- Posts require minimal refinement
- Strong first drafts that need only tuning
- Variety across topics and styles
- Authentic voice throughout
- Scott Adams principles applied from generation, not just revision

**Process Efficiency:**

- Generation → selection → refinement → publish
- Clear labeling enables quick decisions
- Systematic checks prevent repetition
- Documentation captures learnings

-----

## Evolution & Maintenance

**Weekly Review:**

- Assess topic distribution (are we covering variety?)
- Evaluate style effectiveness (which approaches work best?)
- Update THEMES.md with new ideas
- Review banked drafts for publishing readiness
- Refine instructions based on patterns

**Monthly Review:**

- Analyze broader trends
- Identify gaps in thematic coverage
- Consider new style variations
- Update documentation
- Review banked drafts pipeline

**Continuous:**

- Add to THEMES.md as ideas emerge
- Update BLOG_REFINEMENTS.md after each post
- Incorporate feedback immediately
- Refine process based on what's working
- Add strong drafts to BANKED_DRAFTS.md

-----

## Critical Reminders

1. **Always check recent posts first** - Avoid repetition
1. **Label drafts clearly** - [Style Type] - [descriptor]
1. **Provide three genuinely different options** - Topic AND style variety
1. **Ground everything in reality** - Laurent's actual experience and expertise
1. **One clear idea per post** - No trying to say everything at once
1. **Quality over cleverness** - Substance beats performance
1. **Iterate thoughtfully** - Listen to feedback, improve systematically
1. **Apply Scott Adams principles during generation** - Don't wait for revision
1. **Vary ending formats** - Not every post needs to end with a question
1. **Avoid conservative framing** - Laurent's philosophy embraces strategic expansion, not staying small/safe
1. **Mine personal expertise** - Concrete examples from Laurent's life make abstract concepts compelling
1. **Bank strong drafts strategically** - Some concepts deserve careful iteration
1. **Titles matter** - Shorter and punchier usually wins

-----

## Week 5 Key Learnings (Feb 2-8, 2026)

**What Worked:**

- Banked drafts pipeline proving valuable - 2 published this week (Stop Asking, Draw the Line)
- Successfully reworked Lindy Effect with original examples (avoided Taleb echo)
- Sovereignty mini-arc (Stop Asking -> Stay Because You Can Leave -> Draw the Line) creates cohesion
- New concepts (Ratchet Effect, Constraints Build) add fresh territory
- Heinlein quote in "Build the Base First" adds authority without feeling derivative

**What to Avoid:**

- Taleb echo remains the biggest risk - always check examples, not just concepts
- Antifragility draft still needs complete rework (examples too close to source)
- Don't over-rely on banked drafts - new generation keeps things fresh

**High-Value Concepts Still to Develop:**

- Antifragility (BANKED - needs new examples)
- Emergence from simple rules (BANKED - needs personal example)
- Systems vs Goals (BANKED - near-ready)
- Maintenance debt (BANKED - ready to publish)
- Skin in the game (BANKED - space from Jan 29)
- Large organization convergence (Laurent's original concept - needs his framing)

**Patterns Observed:**

- Observational style dominates selections (5 of 7 this week)
- Provocative style works best when grounded in concrete examples
- Mini-arcs (2-3 related posts) create satisfying progression
- Banked drafts benefit from ~1 week of iteration time

---

**Last Updated:** February 8, 2026
**System Version:** v1.3
**Total Posts Published:** 29 consecutive days (Jan 11 - Feb 8, 2026)
**Banked Drafts:** 5 active (4 published to date)
